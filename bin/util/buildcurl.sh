cachecurl() {
	...
	notice_inline "Cachecurl ($1) for binary:$binary in output:$output but to $cache_dir/$binary.tar.gz" 1>&2
	mkdir -p "$cache_dir"
	if [ ! -f $cache_dir/$binary.tar.gz ]; then
		notice_inline "         Downloading Binary for the first time" 1>&2
